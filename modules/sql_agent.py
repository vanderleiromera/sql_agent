...
class SQLQueryCaptureCallback(BaseCallbackHandler):
    ...
    def execute_query_with_optimized_context(self, user_question: str) -> Dict[str, Any]:
        ...
        """
        from .odoo_query_optimizer import OdooQueryOptimizer
        
        # Criar instância do otimizador
        optimizer = OdooQueryOptimizer(
            agent=self.agent, 
        ...
        # Injetar métodos necessários
        optimizer.find_relevant_tables = self.find_relevant_tables
        optimizer.enhance_query_with_table_info = self.enhance_query_with_table_info
        optimizer.get_table_relationships = self.get_table_relationships
        
        # Transferir cache se existir
        if hasattr(self, 'query_cache'):
            optimizer.query_cache = self.query_cache
        
        # Transferir contexto Odoo se existir
        if hasattr(self, 'odoo_context'):
            optimizer.odoo_context = self.odoo_context
            
        # Executar consulta otimizada
        result = optimizer.execute_query_with_optimized_context(user_question)
        
        # Atualizar cache local com o cache do otimizador
        if hasattr(optimizer, 'query_cache'):
            self.query_cache = optimizer.query_cache
